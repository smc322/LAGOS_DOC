...
pptcol<-readRDS("Datasets/JAGS_PPTCOL_mar19.rds")
pptdoc<-readRDS("Datasets/JAGS_PPTDOC_july18.rds")

tempcol<-readRDS("Datasets/JAGS_TMEANCOL_mar19.rds")
tempdoc<-readRDS("Datasets/JAGS_TMEANDOC_mar19.rds")
...
tempcol$var="color"
tempdoc$var="doc"
tempcol$tmeanpctchg=tempcol$slopemean*100
tempdoc$tmeanpctchg=tempdoc$slopemean*100
tc<-tempcol[,c("lagoslakeid", "var", "tmeanpctchg")]
td<-tempdoc[,c("lagoslakeid", "var", "tmeanpctchg")]
...
col.chla.tp.temp<-(merge(col.chla.tp, tc, by="lagoslakeid", all.x=T, all.y=T))
...
doc.chla.tp.temp<-merge(doc.chla.tp, td, by="lagoslakeid", all.x=T, all.y=T)

saveRDS(col.chla.tp.temp, file="Datasets/ColorCovars_Mar19.rds")
saveRDS(doc.chla.tp.temp, file="Datasets/DOCCovars_Mar19.rds")
